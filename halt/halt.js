 ...
 * @returns {WasmGame}
 */
export function create_game(config) {
    const ret = wasm.create_game(config);
 ...
 * @returns {string}
 */
export function make_move(w, mv) {
    let deferred2_0;
    let deferred2_1;
    try {
        ...
        const ret = wasm.make_move(w.__wbg_ptr, ptr0, len0);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('halt_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
